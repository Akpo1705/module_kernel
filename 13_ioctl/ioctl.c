...
static int __init ModuleInit(void) {
	...
	printk("Hello, Kernel!\n");
	/* register device nr. */
	retval = register_chrdev(MYMAJOR, NAME_DEV, &fops);
	if(retval == 0) {
		printk("ioctl_dev - registered Device number Major: %d, Minor: %d\n", MYMAJOR, 0);
	}
	else if(retval > 0) {
...
static void __exit ModuleExit(void) {
	unregister_chrdev(MYMAJOR, NAME_DEV);
